...
void BosonStar::compute(Cell<data_t> current_cell) const
{
    ...
    //Thomas' trick, using the BH metric value in the BSBH case. This works especially well for M_BH < M_BS.
    ...
    if (initial_data_choice == 2 || initial_data_choice == 3)
    {
        ...
        if (BS_BH_binary && initial_data_choice == 2)
        {
            ...
            //metric variables at fixed distance from BH center
            double r_22 = M;
            ...
            //experimental: start from TT metric for correction at BH location instead of plain superposition
           // g_xx += 1 - helferLL[0][0];
           // g_yy += 1 - helferLL[1][1];
           // g_zz += 1 - helferLL[2][2];

	    //superpose_1[0][0] += 1 - helferLL[0][0];
            //superpose_1[1][1] += 1 - helferLL[1][1];
            //superpose_1[2][2] += 1 - helferLL[2][2];



           //without this line h wont have unit determinant unless we start from plain
            ...
            //This is \chi_A(x_A) (old chi at center of star on its own)
            ...
            chi_ = chi_plain + delta_star * R_BS / sqrt(R_BS * R_BS + r_star * r_star);
	    ...
	    chi_ += 1.0 * exp( -10* (r_star - separation) * (r_star - separation) );
        }
...
*/
        ...
        //New one-bump-function (non asymptotically flat) idea. This works well for M_BH >= M_BS
	...
	double profile1 = R_BS / sqrt(R_BS * R_BS + r_star * r_star);       
